		...
		if(usem3==1&&!memcmp(dldiid,"M3DS",4)){
			...
			if(!stat("/defaulty.nds",&st)){
				...
				if(m3sakura_iniclear_method){
					unlink("/defaultn.nds");
					_consolePrintf("iniclear OK. Please restart NDS.\n");
			...
			if(!stat("/!!!extlink",&st)){
				...
				if(!*fullname){
					_consolePrintf("Cannot find recover filename in eos_recover. Halt.\n");die();
				}
...
_consolePrintf("Entered DSTWO mode.\n");
			char dstwodir[768];
			...
			if(!stat("/!!!extlink",&st)){
				char fullname[768],tmpname[768];
_consolePrintf("/!!!extlink exists.\n");
				strcpy(tmpname,"/!!!extlink/");
				char *filename=tmpname+strlen(tmpname);
				ini_gets("Config","dstwo_recover","",fullname,768,"/MOONSHL2/EXTLINK/inilink.ini");
				if(!*fullname){
					_consolePrintf("Cannot find recover filename in dstwo_recover. Halt.\n");die();
				}
				SplitItemFromFullPathAlias(fullname,NULL,filename);
_consolePrintf("start rename.\n");
				rename(tmpname,fullname);
				strcpy(getextname(tmpname),".sav");
				strcpy(getextname(fullname),".sav");
				rename(tmpname,fullname);
				...
				strcpy(getextname(fullname),".dso");
				rename(tmpname,fullname);
				ini_puts("Config","dstwo_recover","","/MOONSHL2/EXTLINK/inilink.ini");
_consolePrintf("start remove.\n");
				rm_rf("/!!!extlink/");
				unlink("/!!!extlink");
_consolePrintf("write ini.\n");
				strcpy(getextname(fullname),".nds");
				strcpy(tmpname,"fat1:");
				getsfnlfn(fullname,tmpname+5,NULL);
				strcpy(dstwodirfilename,"lastsave.ini");
				ini_puts("Dir Info","last dir",fullname,dstwodirfilename);
			}
			//char tmp[776];
			//strcpy(tmp,"fat1:");
			//strcpy(tmp+5,utf8);
			...
			//strcpy(tmp+5,extlink.DataFullPathFilenameAlias);
			//ini_puts("Dir Info","last dir",tmp,"/_dstwo/lastsave.ini");
			//ini_putl("system","enterLastDirWhenBoot",0,"/_dstwo/globalsettings.ini");
			...
			if(!stat("/!!!extlink",&st)){
				...
				if(!*fullname){
					_consolePrintf("Cannot find recover filename in ez_recover. Halt.\n");die();
				}
				SplitItemFromFullPathAlias(fullname,NULL,filename);
				rename(tmpname,fullname);
...
_consolePrintf("Release all keys.\n");

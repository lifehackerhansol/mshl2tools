...
int main(int argc, char **argv){
  ...
  int size=S-0x450,u;
  if(argc<3){puts("r4isdhc in out");return 1;}
  in=fopen(argv[1],"rb");
  ...
  while(u=fread(buf,1,65536,in))fwrite(buf,1,u,out),size-=u;
  fclose(in);
  memset(buf,0,65536);
  while(size>0)fwrite(buf,1,size<65536?size:65536,out),size-=65536;
  fclose(out);
  return 0;
}

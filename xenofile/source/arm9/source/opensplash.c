...
int zlib_decompress(const void *compbuf, const u32 compsize, void *decompbuf, const u32 decompsize){
	...
	for(;;){
		...
		if(z.avail_out == 0){
			printf("deflate buffer overflow.\n");
			return(-1);
		}
	}
...
splash* opensplash(char *path){
	...
	//_consolePrintf2("size1=%d size2=%d real=%d\n",size1total,size2total,filesize);
...
int decompressimage(splash* p, int idx){
	...
	switch(p->version){
		case SPLASH_MOONSHELL1:{
			...
			swiDecompressLZSSWram(p->compbuf,p->decompbuf);
			return 0;
		}
		...
		case SPLASH_MOONSHELL2:{
			vramcpy(p->decompbuf,p->firstbuf,p->head[0].size1);
			if(idx>0){
				int i=0,dest=0;
				fseek(p->f,p->head[idx].offset,SEEK_SET);

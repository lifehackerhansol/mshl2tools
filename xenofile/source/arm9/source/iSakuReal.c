...
int iSakuRealDecode(memstream *comp,memstream *decomp,memstream *decompbuf,u32 palcnt,u16 *pal){
	...
	u32 x=mread32(comp);
	if((x&0xff)!=0xf0){printf("signature error\n");return 1;}
	while(1){
		if(!mavail(comp))break;
		flagscnt--;
		if(flagscnt==0){
			flagscnt=32;
			flags=mread32(comp);
		...
		if(!(flags&0x80000000)){
			...
			offset>>=10; // ...(*)
			if(offset>0x7f){printf("data error offset=%d\n",offset);return 1;}
			...
			if(onepixel){
				...
				for(;idx<(length+3);idx++)mwrite16(pixel,decomp);
			}else{
				for(;idx<(length+3);idx++)mwrite16(mread16(decompbuf),decomp);
			}
		}
	}

...
*/
	...
	printf("Startup is nullified. Might not work (please test certainly). Make sure you launched DLDI Captor from normal OS.\n"); 
done:
	memcpy(pD+magicString,dldimagic,12); //Restore Magic
#endif
...
//	Captor
{
	...
	FILE *f;
	strcpy(dldiname,"/");
	memcpy(dldiname+1,(char*)pD+ioType,4);
	sprintf(dldiname+5,"_%s.dldi",(char*)pD+friendlyName);
	_consolePrintf("Writing to %s\n",dldiname);
	if(!(f=fopen(dldiname,"wb"))){_consolePrintf("Cannot open %s\n",dldiname);return -1;}
...
int main(int argc, char **argv){ //for PC main()
	...
	if(!(pdldi=malloc(stdldi.st_size))){fclose(fdldi);printf("cannot allocate %i bytes for dldi\n",(int)stdldi.st_size);return 3;}

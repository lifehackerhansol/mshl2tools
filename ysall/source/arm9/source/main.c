...
void SoftReset(void)
{
	...
	_consolePrintf("Patching /YSMenu/exlinker.nds... ");
	{
		...
		fread(0x02000000+2*1024*1024,1,st.st_size,f);
		rewind(f);
		dldi(0x02000000+2*1024*1024,st.st_size);
		fwrite(0x02000000+2*1024*1024,1,st.st_size,f);
		fclose(f);
	}
	_consolePrintf("Done.\n");
	...
	{
		u8 *arm9=(u8*)(*(vu32*)0x027FFDF4)+512;
		arm9[0x220e7]=0xea; //yspatch
		strcpy(arm9+0x5a5fc,"exlinker.nds");
		strcpy(arm9+0x5a613,"exlinker.nds");
		strcpy(arm9+0x5a621,"exlinker.nds");

...
void moonshellreset(u8 *pFileBuf)
{
  REG_IME = IME_DISABLE;	// Disable interrupts
  REG_IF = REG_IF;	// Acknowledge interrupt
  
  IPCEX->BootAddress=0;
  IPCEX->RESET=RESET_MENU_GEN;
  ...
  //boot_VRAM_GetBody((u32*)pFileBuf,FileSize,BootAddress);
...
//  _consolePrintf("Reset to 0x%08x\n",BootAddress);
  ...
  *((vu32*)0x027FFE04) = (u32)0xE59FF018;  // ldr pc, 0x027FFE24
  *((vu32*)0x027FFE24) = (u32)0x027FFE04;  // Set ARM9 Loop address
  IPCEX->BootAddress=(u32)pFileBuf;//BootAddress;
  swiSoftReset();  // Reset
...
*/
  ...
  //if(BootType==EBT_None){BootType=EBT_VRAM;}
  ...
  //switch(BootType){
	...
	u8 *pFileBuf=(u8*)malloc(FileSize+64*1024); //64KB more
	if(pFileBuf&&(free(pFileBuf),pFileBuf=(u8*)malloc(FileSize))){
		fread(pFileBuf,1,FileSize,FileHandle);
//dldi(pFileBuf,FileSize);
		_consolePrint("fclose\n");
		fclose(FileHandle);
//FILE *x=fopen("/moonshl/boot.bin","wb");
//fwrite(pFileBuf,1,FileSize,x);
//fclose(x);  
	}
if(!pFileBuf){
	...
	_consolePrintf("Booting\n");
	dldi(pFileBuf,FileSize),moonshellreset(pFileBuf);
    //}break;
    //default: break;
  //}

...
void SoftReset(void)
{
  ...
  //_consolePrintf("dldi version\n");
	...
	_consolePrintf(
		"MoonShell2 yslaunch aka ysloader multibyte\n"
	...
	_consolePrintf("Configuring... ");
{
	...
	ini_gets("Config","YSMenu","/YSMENU/YSMENU.NDS",ysmenu,768,"/MOONSHL2/EXTLINK/CONFIG.INI");
	ini_gets("Config","YSini","/YSMENU/YSMENU.INI",ininame,768,"/MOONSHL2/EXTLINK/CONFIG.INI");
...
/*
{
	...
	FILE *f=fopen(ysmenu,"rb");
	fseek(f,0,SEEK_END);
	FileSize=ftell(f);
	fseek(f,0,SEEK_SET);
	pFileBuf=(u8*)malloc(FileSize);
	if(!f||!FileSize||!pFileBuf){_consolePrintf("size=%d buf=%08x\n",FileSize,pFileBuf);while(1);}
	fread(pFileBuf,1,FileSize,f);
	dldi(pFileBuf,FileSize);
	//IPCEX->BootAddress=(u32)pFileBuf;
	fclose(f);
}
*/
extern void moonshellreset(u8 *pFileBuf);
	//RESET=RESET_MENU_GEN;
	//IPCEX->RESET=RESET;
	_consolePrintf("Rebooting... \n");
	//ret_menu9_GENs();
	//moonshellreset(pFileBuf);
BootNDSROM(ysmenu);
